...
| **层级**（tier） | 数字越小越优先。只在最小可用层里挑；整层都被冷却/熔断才降级。**是分组不是序号**：同层的条目按 `hash(会话, 条目, key)` 摊给不同会话，层内没有先后 |
...
「条目总表」页把所有渠道的条目铺进**一张表**，按层分段——渠道页是按渠道分块的，看不到全局顺序，
调优先级只能逐个开编辑框填数字，而层级是分组不是序号，填出来常常和直觉相反。总表里拖一行的手柄
可以换层，也可以用 ↑↓ 或「插入新层」（勾哪几个条目，它们就组成新的一层；同一个对话框也能拆层）。
只有**跨层**才影响路由：层内本来就按会话哈希摊开，页上排的先后在那一层不起作用，所以在层里拖会被
拒绝而不是给你一个假生效的手势。每次调整提交一次 `POST /admin/api/entries/reorder`，服务端按提交的层
重写 `tier`（步长 10，中间永远留得下插新层的空当），写完热加载配置快照，即刻生效不用重启。

搜索后端的顺序**不用另开一页**，「搜索」页上直接排：那里的顺序本来就是运行期语义
（`search_loop` 从上往下试，第一个搜到东西的就算数，后面的根本不会被打扰），以前只能按 id 排、
调不了。拖行首的手柄或用 ↑↓ 改，提交 `POST /admin/api/search_backends/reorder` 重写 `sort_order`。
和条目不同的是这里**每次搜索都现读库**，所以不用等热加载，写完下一次搜索就生效；新加的后端自动
排在最后——刚添的备用通道不该插到已经调好的顺序前面去。
