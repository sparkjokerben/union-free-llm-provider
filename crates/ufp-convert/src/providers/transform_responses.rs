...
fn validate_anthropic_web_search_direct_mode(tool: &Value) -> Result<(), ConvertError> {
    let tool_type = tool
        .get("type")
        .and_then(Value::as_str)
    ...
    match tool.get("allowed_callers") {
        ...
        None => Err(ConvertError::InvalidRequest(format!(
        ...
        Some(_) => Err(ConvertError::InvalidRequest(
...
fn anthropic_web_search_to_responses(
    tool: &Value,
    is_codex_oauth: bool,
) -> Result<(Value, Option<u64>), ConvertError> {
...
fn convert_messages_to_input(messages: &[Value]) -> Result<Vec<Value>, ConvertError> {
...
pub fn responses_to_anthropic(body: Value) -> Result<Value, ConvertError> {
...
pub(crate) fn responses_to_anthropic_with_web_search_options(
    ...
    let output = body
        ...
        .ok_or_else(|| ConvertError::TransformError("No output in response".to_string()))?;
            ...
            "function_call" => {
                ...
                let input: Value = if args_str.trim().is_empty() {
                    ...
                    match serde_json::from_str(args_str) {
                        ...
                        Err(error) => {
                            return Err(ConvertError::TransformError(format!(
...
mod tests {
    ...
    fn test_completed_function_call_invalid_arguments_is_error() {
        ...
        assert!(matches!(
            ...
            Err(ConvertError::TransformError(_))
        ));
    }
    ...
    fn test_responses_failed_status_is_not_silent_empty_success() {
        ...
        assert!(
            matches!(error, ConvertError::TransformError(message) if message.contains("backend exploded"))
        );
    }
    ...
    fn test_responses_error_envelope_preserves_upstream_message() {
        ...
        let error = responses_to_anthropic(input).unwrap_err();
        assert!(
            matches!(error, ConvertError::TransformError(message) if message.contains("too many requests"))
        );
    }
    ...
    fn test_responses_cancelled_status_is_not_end_turn() {
        ...
        let error = responses_to_anthropic(input).unwrap_err();
        assert!(
            matches!(error, ConvertError::TransformError(message) if message.contains("cancelled"))
        );
    }
