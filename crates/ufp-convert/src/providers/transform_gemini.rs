...
#[allow(dead_code)]
pub fn anthropic_to_gemini(body: Value) -> Result<Value, ConvertError> {
...
pub fn gemini_to_anthropic(body: Value) -> Result<Value, ConvertError> {
...
pub fn gemini_to_anthropic_with_shadow_and_hints(
    ...
    let candidate = body
        ...
        .ok_or_else(|| {
            ConvertError::TransformError("No candidates in Gemini response".to_string())
        })?;
...
fn collect_system_texts(value: &Value, texts: &mut Vec<String>) -> Result<(), ConvertError> {
    ...
    let Some(blocks) = value.as_array() else {
        return Err(ConvertError::TransformError(
        ...
        match block_type {
            ...
            "image" => {
                let source = block.get("source").ok_or_else(|| {
                    ConvertError::TransformError("Gemini image block missing source".to_string())
                })?;
            ...
            "document" => {
                let source = block.get("source").ok_or_else(|| {
                    ConvertError::TransformError("Gemini document block missing source".to_string())
                })?;
            ...
            "tool_result" => {
                ...
                let name = tool_name_by_id
                    ...
                    .ok_or_else(|| {
                        ConvertError::TransformError(format!(
...
fn map_tool_choice(tool_choice: Option<&Value>) -> Result<Option<Value>, ConvertError> {
