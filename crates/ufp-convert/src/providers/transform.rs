...
pub fn openai_to_anthropic(body: Value) -> Result<Value, ConvertError> {
    let choices = body
        ...
        .ok_or_else(|| ConvertError::TransformError("No choices in response".to_string()))?;

    let choice = choices
        .first()
        .ok_or_else(|| ConvertError::TransformError("Empty choices array".to_string()))?;

    let message = choice
        .get("message")
        .ok_or_else(|| ConvertError::TransformError("No message in choice".to_string()))?;
...
mod tests {
    ...
    fn test_openai_to_anthropic_preserves_id_for_usage_dedup() {
        ...
        let result = openai_to_anthropic(input).unwrap();
        let usage = crate::usage::parser::TokenUsage::from_claude_response(&result)
